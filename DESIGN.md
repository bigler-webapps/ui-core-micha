...
10. **Prototype = spec, parity = guardrail — in BOTH directions.** Purely visual/UX — function stays. **"No behaviour / permission / data-contract change"** is the hard condition that goes into the WO. **What the prototype OMITS is also spec:** on a redesign of an existing screen, whatever is there today and absent from the prototype is meant to GO — the silence is an instruction, not a gap. So parity is checked both ways: everything in the prototype is present, AND nothing outside it survives on that screen. **A redesign that ends up net-additive has failed**, however good its new parts are — shipping MORE UI than before is the estate's recurring cleanup failure (extra status bands, duplicated badges, tip text layered on top of the legacy the redesign was meant to replace).
